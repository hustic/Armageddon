...
class Planet():
    ...
    def rhoa(x):
        if atmos_func == 'exponential':
            return lambda x: rho0 * np.exp(-x/self.H)
    ...
    def solve_atmospheric_entry(
        ...
        init_distance = np.zeros(len(radius)) # intial distance assumed to be zero
        y = pd.DataFrame({
            'vel': [velocity],    
            'mass': [mass],
            'angle': [angle], 
            'alt': [init_altitude],
            'dist': [init_distance],
            'rad': [radius]}) # defining initial condition Frame
        print(y)
        ...
        while t <= T: # initiate timeloop
            ...
            i = np.arange(0, len(strength))
            if strength[i] <= (self.rhoa((y['alt'][i]) * y['vel'][i]**2)) and fragmentation is True:
                fragmented[i] = True # define status of fragmentation
            else:
                fragmented = False
